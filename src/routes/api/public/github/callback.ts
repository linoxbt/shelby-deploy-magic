  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (!code || !state)
          return Response.redirect(
            `${origin}/dashboard?github_error=Missing%20GitHub%20callback%20data`,
            302,
          );
        ...
        try {
          ...
          const message =
            callbackError instanceof Error ? callbackError.message : "GitHub connection failed";
          return Response.redirect(
            `${origin}/dashboard?github_error=${encodeURIComponent(message)}`,
            302,
          );
        }
      },
