    ...
    <AppShell>
      <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="font-mono text-sm font-semibold uppercase text-primary">ShelbyHost dashboard</p><h1 className="mt-2 text-3xl font-extrabold text-foreground">Good morning, builder.</h1></div>
          <div className="flex flex-col gap-2 sm:flex-row"><button onClick={() => connectWallet("aptos")} className="inline-flex items-center justify-center gap-2 rounded-md border border-primary/40 px-4 py-2.5 text-sm font-bold text-primary transition hover:bg-primary/10"><Wallet className="h-4 w-4" /> {wallet ? "Aptos Connected" : "Connect Aptos"}</button><Link to="/deploy" className="inline-flex items-center justify-center gap-2 rounded-md bg-primary px-4 py-2.5 text-sm font-bold text-primary-foreground transition hover:bg-primary-hover hover:shadow-glow"><Plus className="h-4 w-4" /> New Deployment</Link></div>
        </header>

        <section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {stats.map((stat) => {
            const Icon = stat.icon;
            return <div key={stat.label} className="rounded-lg border border-border bg-card p-5"><div className="flex items-center justify-between"><p className="text-sm font-medium text-muted-foreground">{stat.label}</p><Icon className="h-4 w-4 text-primary" /></div><p className="mt-4 font-mono text-3xl font-bold text-primary">{stat.value}</p></div>;
          })}
        </section>

        <section className="mt-8 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-lg border border-border bg-card p-5">
            <div className="flex items-center gap-3"><Github className="h-5 w-5 text-primary" /><h2 className="text-lg font-bold text-foreground">GitHub OAuth connect</h2></div>
            <p className="mt-2 text-sm text-muted-foreground">Pick a repo and branch, then ShelbyHost installs a GitHub Action that calls the deploy API on push.</p>
          ...
          <div className="rounded-lg border border-border bg-card p-5">
            <h2 className="text-lg font-bold text-foreground">Deployments history</h2>
        ...
        <section className="mt-10">
          <div className="mb-5"><h2 className="text-xl font-bold text-foreground">Projects</h2><p className="mt-1 text-sm text-muted-foreground">Live frontends resolved by slug, hash, GitHub Action, and optional custom domain.</p></div>
